...
public class LibraryDAO {
	...
	// 프로그램 종료시 C드라이브에서 파일 저장 및 실행시 파일 불러오기 위한 경로 지정
	private static final String USER_FILE = "C:\\fileIO\\users.data"; // 사용자 정보를 저장할 파일 경로 및 파일 이름
	private static final String BOOK_FILE = "C:\\\\fileIO\\\\books.data"; // 도서 정보를 저장할 파일 경로 및 파일 이름
	private static final String CHECKOUT_FILE = "C:\\\\fileIO\\\\checkOuts.data"; // 도서 대출 정보를 저장할 파일 경로 및 파일 이름
	...
	public String deleteBook(String bookNo) {
		...
		// 삭제할 도서의 정보를 append
		sb.append(String.format("%n[%s/%s]가 삭제되었습니다.", b.getBookNo(), b.getBookTitle()));
		// 키를 통해 해당 객체 삭제
	...
	public String viewCheckedOutBooks() {
		...
		if (this.checkOuts.size() != 0) {
			...
			for (CheckOut c : checkOuts) {
				...
				if ((b.getBookStatus().equals("대출중") || b.getBookStatus().equals("연체중"))
						&& !c.getDueDate().equals("")) {
	...
	public String viewUserInDetail(String userNo) {
		...
		sb.append(String.format("%n[%s]의 상세보기 입니다.%n", this.users.get(userNo)));
		sb.append(String.format("오늘 날짜 : %s%n", this.nowDate));
		sb.append("---------------------------------------------------------------------------------------\n");
	...
	public String viewBookInDetail(String bookNo) {
		...
		sb.append(String.format("%n[%s/ %s/ %s/ %s]%n", book.getBookNo(), book.getBookTitle(), book.getPublisher(),
				book.getAuthor()));
		...
		// 만약 체크아웃 리스트 목록이 비어있으면
		if (this.checkOuts.isEmpty()) {
			sb.append("대출 목록이 존재하지 않습니다. \n");
		}
